...
let handler = async (m, { conn, args, usedPrefix, command }) => {
        ...
        if (!/audio/.test(mime)) throw `Falta vn/audio responde a un audio para acortarlo usando *${usedPrefix + command}*`
        let audio = await q.download?.()
        if (!audio) throw 'No se puede descargar el audio!'
        if (!args[0] || !args[1]) throw `Uso correcto: ${usedPrefix + command} 00:00:30 00:00:30`
